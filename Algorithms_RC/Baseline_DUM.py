...
def get_recommendations_dum_chunk(ratings_chunck, items_genres_matrix):
...
def get_recommendation(k_param, ratings, items_items_distances, items_genres_matrix, dataset_name, regime):
    ...
    for i in range(len(chunk_range)):
        ...
        result = pool.apply_async(get_recommendations_dum_chunk,
                                  args=(ratings_chunk, items_genres_matrix))
...
def main(dataset_name):
    ...
    for regime in regimes:
        ...
        k_param = 2
        start = time.time()
        exp_avg, exp_std, recommendation_list = get_recommendation(k_param, ratings, items_items_distances, items_genres_matrix,dataset_name, regime)
